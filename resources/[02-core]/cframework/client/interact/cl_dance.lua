...
RegisterCommand(T("COMMANDS_DANCE"), function(source, args, rawCommand)
    if args[1] then
        TriggerEvent('dances:dance', tonumber(args[1]))
    else
        ESX.ShowNotification(T("DANCES_INVALID"), 'error')
    end
end, false)
...
RegisterNetEvent('dances:dance')
AddEventHandler('dances:dance', function(pDance)
    if not isHandcuffed and not isSoftCuffed then
        local noAnimations = #animations

        if pDance == -1 then
            pDance = math.random(noAnimations)
            ESX.ShowNotification((T("DANCES_RANDOM")):format(pDance), 'inform')
        end

        if pDance > noAnimations or pDance <= 0 then
            ESX.ShowNotification((T("DANCES_LIMIT")):format(noAnimations), 'error')
            return
        end

        if animations[pDance] and animations[pDance].disabled then
            ESX.ShowNotification(T("DANCES_DISABLED"), 'error')
            return
        end

...
RegisterNetEvent('animation:chair', function()
...
function sitRedux(pChairEntity, pChairModel)
    ...
    if sittingChair then
        ...
        for _, chair in pairs(Sitable) do
...
RegisterNetEvent("np-emotes:sitOnChair", function(pEntity, pContext)
  sitRedux(pEntity or nil, pContext or nil)
end)

local oldLocation = vector3(0, 0, 0)

RegisterNetEvent("turnoffsitting", function()
	sitting = false
end)

RegisterCommand(T("COMMANDS_SIT"), function()
    local PlayerPed = PlayerPedId()
    local entity, entityType, entityCoords = GetEntityPlayerIsLookingAt(3.0, 0.2, 286, PlayerPed)

    if entityType == nil or entity == nil then return end

    TriggerEvent("np-emotes:sitOnChair", entity, GetEntityModel(entity))
end, false)
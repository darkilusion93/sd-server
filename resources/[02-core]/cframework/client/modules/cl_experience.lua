...
local RockstarRanks = {800, 2100, 3800, 6100, 9500, 12500, 16000, 19800, 24000, 28500, 33400, 38700, 44200, 50200, 56400, 63000, 69900, 77100, 84700, 92500, 100700, 109200, 118000, 127100, 136500, 146200, 156200, 166500, 177100, 188000,
199200, 210700, 222400, 234500, 246800, 259400, 272300, 285500, 299000, 312700, 326800, 341000, 355600, 370500, 385600, 401000, 416600, 432600, 448800, 465200, 482000, 499000, 516300, 533800, 551600, 569600, 588000, 606500, 625400, 644500,
663800, 683400, 703300, 723400, 743800, 764500, 785400, 806500, 827900, 849600, 871500, 893600, 961600, 984700, 1008100, 1031800, 1055700, 1079800, 1104200, 1128800, 1153700, 1178800, 1204200, 1229800, 1255600, 1281700, 1308100, 1334600,
1361400, 1388500, 1415800, 1443300, 1471100, 1499100, 1527300, 1555800, 1584350, 1612950, 1641600, 1670300, 1699050, 1727850, 1785600, 1814550, 1843550, 1872600, 1901700, 1930850, 1960050, 1989300, 2018600, 2047950, 2077350, 2106800,
2136300, 2165850, 2195450, 2225100, 2254800, 2284550, 2314350, 2344200, 2374100, 2404050, 2434050, 2464100}

--===================================================================================
-- "Settings" you can change
--===================================================================================
local XNL_UseRedBarWhenLosingXP = true	-- This sets if the bar should turn red when a player is LOSING XP (due to an admin 'punishment' for example!)
local XNL_MaxPlayerLevel = 500 			-- Sets the maximum level (Maximum which can be used is 7999!)
									-- This will also prevent the player from getting more XP than possible
									-- IF you use my variable to set/read the player XP
local XNL_EnableZKeyForRankbar = false		-- This Enables/Disables (when you set it to false) the Rankbar showing up when the player presses Z on the keyboard							 
...
RegisterCommand("XNLSetBaseXPExample",function(source, args)
	local FirstParam = args[1]
	TestNR = tonumber(FirstParam)
	XNL_SetInitialXPLevels(TestNR, false, false)
end)

RegisterCommand("setBaseMudaValor",function(source, args)
	local FirstParam = args[1]
	TestNR = tonumber(FirstParam)
	XNL_SetInitialXPLevels(TestNR, false, false)
...
ESX.GetCurrentExperience = function(type)
    if ESX.PlayerData.experience[type] == nil then
        ESX.PlayerData.experience[type] = 0
    end

    return ESX.PlayerData.experience[type]
end

ESX.GetCurrentExperienceLevel = function(type)
    if ESX.PlayerData.experience[type] == nil then
        ESX.PlayerData.experience[type] = 0
    end

    return XNL_GetLevelFromXP(ESX.PlayerData.experience[type])
end

ESX.GetCurrentLevelFromExperience = function(experience)
    return XNL_GetLevelFromXP(experience)
end
...
RegisterNetEvent("cframework:addExperience", function(type, amount)
	...
	ESX.PlayerData.experience[type] = ESX.PlayerData.experience[type] + numAmount
end)

RegisterNetEvent("cframework:setExperience", function(type, value)
	ESX.PlayerData.experience[type] = value
end)

RegisterNetEvent("cframework:giveExperience", function(base, gain)
	XNL_SetInitialXPLevels(base, false, false)
	XNL_AddPlayerXP(gain)
end)

local Components = {
	{label = _U('sexo'),					name = 'sex',				value = 0,		min = 0,	zoomOffset = 0.6,		camOffset = 0.65},
...
function LoadDefaultModelSync(malePed)
	local playerPed = PlayerPedId()
	local characterModel
...
function ApplySkin(skin, clothes)
	...
	TriggerEvent("cframework:skinChanged", Character)
end
...
function LoadSkinSync(skin)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			LoadDefaultModelSync(true)
		else
			LoadDefaultModelSync(false)
		end
	else
		ApplySkin(skin)
	end

	TriggerEvent('cframework:loadTattoos')

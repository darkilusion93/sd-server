...
RegisterNetEvent("fx:run", function(pFXName, pFXTime, pAlcoholStrength, pIsBadLSD, pIsAbsinthe)
...
RegisterNetEvent("fx:clear", function()
  ClearTimecycleModifier()
  ClearExtraTimecycleModifier()
  ShakeGameplayCam('DRUNK_SHAKE', 0.0)
  SetPedMotionBlur(PlayerPedId(), false)
...
RegisterNetEvent('fx:toggleDrunkCamera', function(state)
    state = string.lower(state)
    if state == "on" then
        enableDrunkCamera = true
        TriggerEvent('DoLonchudText', "Drunk camera is now enabled.")
    elseif state == "off" then
        enableDrunkCamera = false
        if isDrunk then
          ShakeGameplayCam('DRUNK_SHAKE', 0.0)
        end
        TriggerEvent('DoLonchudText', "Drunk camera is now disabled.")
    else
        TriggerEvent('DoLonchudText', "You need to type in 'on' or 'off'.")
    end
end)

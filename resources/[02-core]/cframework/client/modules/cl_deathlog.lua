	...
	while true do
		...
		if IsEntityDead(GetPlayerPed(PlayerId())) then
			...
			elseif (Killer == nil) then
				...
				elseif ESX.GetWeaponFromHash(DeathCauseHash) ~= nil then
					DeathReason = 'weapon'
			...
			local bleeding = " - 1"
			local area = T("RESPAWN_AFFECTED_MEMBERS")
			if bone == 31086 then
				bleeding = " - 4"
				area = T("RESPAWN_AFFECTED_HEAD")
				if ESX.DoesWeaponKill(DeathCauseHash) then
					bleeding = " - 5"
				end
			elseif ESX.DoesWeaponKill(DeathCauseHash) then
				bleeding = " - 5"
				area = T("RESPAWN_AFFECTED_TORSO_WITH_BULLETS")
			elseif bone == 24817 or bone == 24818 or bone == 10706 or bone == 24816 or bone == 11816 then
				bleeding = " - 2"
				area = T("RESPAWN_AFFECTED_TORSO")
			end

			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('playerDied', 1, 0, DeathReason, Weapon, area .. bleeding)
			else
				TriggerServerEvent('playerDied', 2, GetPlayerServerId(Killer), DeathReason, Weapon, area .. bleeding)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
...
function IsPistol(Weapon)
	local Weapons = {'WEAPON_SNSPISTOL', 'WEAPON_HEAVYPISTOL', 'WEAPON_VINTAGEPISTOL', 'WEAPON_PISTOL', 'WEAPON_APPISTOL', 'WEAPON_COMBATPISTOL', 'WEAPON_MACHINEPISTOL', 'WEAPON_PISTOL_MK2', 'WEAPON_DOUBLEACTION'}
	for i, CurrentWeapon in ipairs(Weapons) do
		if GetHashKey(CurrentWeapon) == Weapon then
			return true
		end

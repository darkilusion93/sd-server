...
let storedTotalPages = 0;
let totalPages = 0;
let currentPage = 1; // Assuming "page" is the current active page
let maxVisiblePages = 7; // Number of pages visible at a time
...
function buildPagination() {
    ...
    function createPageItem(pageNum, isActive = false) {
        return `<li class="page-item ${isActive ? "active" : ""}">
                    <a class="pagination-page ${isActive ? "active active-pagination" : ""} " data-page="${pageNum}">${pageNum}</a>
                </li>`;
    }
    
    // Always show the first page
    $(".pagination").append(createPageItem(1, currentPage === 1));
    
    let start, end;
    if (totalPages <= maxVisiblePages + 2) {
        // Case: Few pages, show all
        start = 2;
        end = totalPages - 1;
    ...
    // Show "..." before middle numbers if necessary
    if (start > 2) {
        $(".pagination").append('<li class="page-item disabled"><span class="pagination-page disabled">...</span></li>');
    }
    ...
    // Show "..." after middle numbers if necessary
    if (end < totalPages - 1) {
        $(".pagination").append('<li class="page-item disabled"><span class="pagination-page disabled">...</span></li>');
    }
    ...
    if (totalPages > 1) {
        $(".pagination").append(createPageItem(totalPages, currentPage === totalPages));
    }
}
...
function searchList(w, input) {
    if (curwin == "banlist") {
        ...
        $.post(`http://gbans/getListData`, JSON.stringify({ list: curwin, page: page, search: input }), function(data) {
            $(".loader").hide();
            if (data === undefined) return;
            $.each(JSON.parse(data), function(i, item) {
                var expired = (item.length > 0 && item.length < Date.now()) || item.unbanned,
                    receiver = JSON.parse(item.receiver);
                $("<tr" + (expired ? " class='text-mute-custom'" : "") + ">").append(
                    $("<td>").text(item.id),
...
//                    $("<td>").html("<a data-toggle='collapse' data-target='#reccol" + i + "' aria-expanded='false'><span>" + (item.receiver_name || "UNKNOWN") + " (" + receiver[0] + ")</span> <i class='arrow'></i></a><br><div class='collapse' id='reccol" + i + "'>" + receiver.slice(1, receiver.length).join("<br>") + "</div>"),
            ...
            if (input === "") {
                totalPages = parseInt(storedTotalPages);
            } else {
...
$("#banbtn").click(function() {
    ...
    if (target == 0 || target == "" || reason == "" || (length == "" && !perm)) {
        ...
        $.post(`http://gbans/ban`, JSON.stringify({ target: target, reason: reason, length: perm ? null : length, offline: offline }));
    }
});
...
$("#warnbtn").click(function() {
    ...
    if (target == 0 || target == "" || message == "") {
        ...
        $.post(`http://gbans/warn`, JSON.stringify({ target: target, message: message, anon: anon }));
    }
});
...
$("body").on("click", "#unbanbtn", function() {
    $.post(`http://gbans/unban`, JSON.stringify({ id: $(this).data("id") }));
    $(this).parent().parent().addClass("text-mute-custom");
...
$("body").on("click", ".pagination-page", function() {
    if (curwin != "banlist" && curwin != "warnlist" || $(this).hasClass("active") || $(this).hasClass("disabled")) return;
    page = $(this).data("page");
    currentPage = page; // Assuming "page" is the current active page
    buildPagination();
    ...
    $(".loader").show();
    if (curwin == "banlist") {
        $.post(`http://gbans/getListData`, JSON.stringify({ list: curwin, page: page, search: "" }), function(data) {
            $(".loader").hide();
            if (data === undefined) return;
            $.each(JSON.parse(data), function(i, item) {
                var expired = (item.length > 0 && item.length < Date.now()) || item.unbanned,
...
$(function() {
    ...
    document.onkeyup = function(data) {
        if (data.which == 27) {
            toggle();
            $.post(`http://gbans/hidecursor`, JSON.stringify({}));
        }
    };
    ...
    window.addEventListener('message', function(event) {
        ...
        if (event.data.show) {
            ...
            if (event.data.window == "ban" || event.data.window == "warn") {
                ...
                storedTotalPages = parseInt(event.data.pages);
                totalPages = parseInt(event.data.pages);
                currentPage = page; // Assuming "page" is the current active page
                ...
                for (i = 1; i < parseInt(event.data.pages) + 1; i++) {
                    $(".pagination").append('<li class="page-item"><a class="pagination-page ' + (i == page ? "active" : "") + '" data-page="' + i + '">' + i + '</a></li>');
                }
                $.each(JSON.parse(event.data.list), function(i, item) {
                    $("<tr>").append(
                        $("<td>").text(item.id),

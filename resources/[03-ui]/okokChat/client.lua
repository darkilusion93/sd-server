...
RegisterCommand('gethash', function()
    ...
    local target = coords + (forward * 5.0) -- Procura num raio de 5 metros à tua frente
    
    local _, hit, endCoords, _, entity = GetShapeTestResult(StartShapeTestRay(coords.x, coords.y, coords.z, target.x, target.y, target.z, -1, playerPed, 0))
    
    if hit == 1 and entity ~= 0 then
        local model = GetEntityModel(entity)
        print("--- DEBUG DE OBJETO ---")
        print("Hash (número): " .. model)
        print("Nome do Modelo: " .. GetDisplayNameFromVehicleModel(model) or "N/A") -- Funciona para alguns objetos
        print("-----------------------")
    else
        print("Nenhum objeto encontrado à tua frente.")
    end
end)

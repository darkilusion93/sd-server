...
RegisterCommand('cadeirarodas', function()
	if PlayerData.job.name == 'ambulance' then
		...
		local wheelchair = CreateObject(GetHashKey('prop_wheelchair_01'), coords.x+0.9,coords.y,coords.z, true)											
		PlaceObjectOnGroundProperly(wheelchair)
	end
end, false)

RegisterCommand('tirarcadeira', function()
	if PlayerData.job.name == 'ambulance' then
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				DrawText3Ds(pickupCoords, "[E] Empurrar", 0.4)
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Alguém já está a usar a cadeira de rodas")
			return
		end
	end

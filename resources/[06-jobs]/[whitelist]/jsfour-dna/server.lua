...
-- FIX (2026-06-12): todos os eventos de ADN exigem job de polícia/forense.
-- Antes qualquer jogador forjava/apagava provas e lia registos.
...
AddEventHandler('jsfour-dna:fetch', function( type )
  ...
  if not isCop(_source) then return end
  local data = {}
...
AddEventHandler('jsfour-dna:save', function( dna )
  local _source = source
  if not isCop(_source) then return end
  if type(dna) ~= "table" then return end
  local pTarget = ESX.GetPlayerFromId(dna.p)
  if not pTarget then return end
  local identifier = pTarget.identifier
  local datum = os.date("%Y-%m-%d")

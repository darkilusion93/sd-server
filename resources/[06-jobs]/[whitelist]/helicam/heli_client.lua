...
local valkyrie_hash = GetHashKey("annihilator")
local fire_hash = GetHashKey("firehawk")
local fire2_hash = GetHashKey("lguardmav")
local fire2b_hash = GetHashKey("frogger3")
local fire3_hash = GetHashKey("aw139b")
local fire4_hash = GetHashKey("aw139bmar")
local fire5_hash = GetHashKey("annihilator2")
local fire6_hash = GetHashKey("cargobob3")
local fire7_hash = GetHashKey("f16f")
local fire8_hash = GetHashKey("annihilator3")

local ops_hash = GetHashKey("as350")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision

local rapel_type = false
	...
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) and rapel_type == true then -- Initiate rappel
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				if manual_spotlight then -- Continuously update manual spotlight direction, sync client-client with decorators
					...
					if Mspotlight_toggle == true then
						TriggerServerEvent("heli:manual.spotlight.toggle")
					end
				end
...
function IsPlayerInPolmav()
	...
	if IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, valkyrie_hash) or IsVehicleModel(vehicle, fire_hash) or IsVehicleModel(vehicle, fire2_hash) or IsVehicleModel(vehicle, fire2b_hash) or IsVehicleModel(vehicle, fire3_hash) or IsVehicleModel(vehicle, ops_hash) or IsVehicleModel(vehicle, fire4_hash) or IsVehicleModel(vehicle, fire5_hash) or IsVehicleModel(vehicle, fire8_hash) then
		rapel_type = true
		return true
	elseif (IsPedInAnyHeli(PlayerPedId()) and not IsVehicleModel(vehicle, fire6_hash)) or IsVehicleModel(vehicle, fire7_hash) then
		local matri_pj = GetVehicleNumberPlateText(vehicle)
		
		if matri_pj == 'MARINHA ' then					
			rapel_type = false
			return true
		else
			return false			
...
function RenderVehicleInfo(vehicle)
	if DoesEntityExist(vehicle) then
		...
		if speed_measure == "MPH" then
			...
			vehspeed = GetEntitySpeed(vehicle)*3.6 * 1.1
		end
		SetTextFont(0)
		...
		SetTextEntry("STRING")
		if vehicle_display == 0 then
			AddTextComponentString("Velocidade: " .. math.ceil(vehspeed) .."km/h")
		elseif vehicle_display == 1 then

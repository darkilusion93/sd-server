...
local nomeagente_temp = nil
local ocorrencia_temp = nil
...
RegisterNUICallback("laboratorio", function(data, cb)
    local tipo = data.tipo
	local label = data.label
    
    exports['okokNotify']:Alert('', 'Chamada '..label..' Feita com sucesso', 5000, 'info', true)
	TriggerEvent('qs-smartphone:client:chamadamanualmente', tipo, label)
end)
...
RegisterNetEvent('np-mdt:abrir')
AddEventHandler('np-mdt:abrir', function()
    ExecuteCommand('siip')
end)
...
AddEventHandler('np-mdt:open', function(jobLabel, name)
    ...
    SendNUIMessage({ type = "data", name = "Bem Vindo, " ..jobLabel..' '.. name, location = playerStreetsLocation, fullname = name })
end)
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    local id = data.id
    local joblabel = data.job
    TriggerServerEvent('np-mdt:searchProfile', name,id,joblabel)
    cb(true)
end)
...
AddEventHandler('np-mdt:searchVehicles', function(sentData)
    for i=1, #sentData do
        ...
        sentData[i]["id"] = sentData[i].nif
        sentData[i]['plate'] = string.upper(sentData[i]['plate'])
        sentData[i]['color'] = ColorInformation[vehicle['color1']] or 'white'
        sentData[i]['colorName'] = ColorNames[vehicle['color1']] or 'Cor Desconhecida'
...
AddEventHandler('np-mdt:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        ...
        vehicle["id"] = sentData[1].nif
        vehicle['color'] = ColorInformation[vehData['color1']] or 'white'
        vehicle['colorName'] = ColorNames[vehData['color1']] or 'Cor Desconhecida'
...
AddEventHandler('np-mdt:getActiveUnits', function(lspd, bcso, sahp, sasp, doc, sapr, pa, ems, doj)
    SendNUIMessage({ type = "getActiveUnits", lspd = lspd, bcso = bcso, sahp = sahp, doc = doc, sasp = sasp, sapr = sapr, pa = pa, ems = ems, doj = doj })
end)
...
AddEventHandler('np-mdt:callDetach', function(callid, sentData)
    ...
    if job.name == "police" or job.name == 'sheriff' or job.name == 'state' or job.name == 'ranger' or job.name == 'municipal' then SendNUIMessage({ type = "callDetach", callid = callid, data = tonumber(sentData) }) end
end)
...
AddEventHandler('np-mdt:callAttach', function(callid, sentData)
    local job = ESX.GetPlayerData().job
    if job.name == "police" or job.name == 'sheriff' or job.name == 'municipal' or job.name == 'ranger' or job.name == 'state' then
...
--     local job = ESX.GetPlayerData().job
--     if job.name == "police" or job.name == 'sheriff' or job.name == 'state' or job.name == 'municipal' or job.name == 'ranger' then
...
AddEventHandler('np-mdt:dashboardMessage', function(sentData)
	...
	if sentData.job == 'police' and job == 'sheriff' then
		SendNUIMessage({ type = "dispatchmessage", data = sentData })
	elseif job == sentData.job then
        SendNUIMessage({ type = "dispatchmessage", data = sentData })
    end
end)
...
RegisterNUICallback("enviaremail", function(data, cb)
    local cid = data.id
    local fName = data.fName
    local sName = data.sName
    TriggerServerEvent("np-mdt:enviarEmail", cid, fName, sName, tempname)
    cb(true)
end)
...
RegisterCommand('gerar_auto', function(source, args)
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)
	local street = GetStreetNameAtCoord(coords.x, coords.y, coords.z)
    local street1 = GetStreetNameFromHashKey(street)
	if nomeagente_temp ~= nil and ocorrencia_temp ~= nil then
		TriggerServerEvent('autocontra:go', tonumber(ocorrencia_temp),nomeagente_temp,street1)
	else
		exports['mythic_notify']:SendAlert('error','Nenhum auto detetado!')
	end
end, false)

RegisterNUICallback('verAV', function(data, cb)
	Citizen.Wait(200)
	TriggerServerEvent('autocontra:buscaAV',string.sub(data.id, 4,20))
end)
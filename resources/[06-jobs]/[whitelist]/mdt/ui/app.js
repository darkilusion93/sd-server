...
$(document).ready(() => {
    ...
    $(".pspc2").click(function() {
		
		showLSPD = !showLSPD
		alterarvisao()
	
	})
	$(".gnrc2").click(function() {
	...
	$(".marc2").click(function() {
		
		showSAHPPM = !showSAHPPM
		alterarvisao()	
	})
    $(".lpcon1").click(function() {
		$.post(`https://${GetParentResourceName()}/laboratorio`, JSON.stringify({
                        tipo: 'pj',
						label: 'LPC',
        }));
    ...
    $(".bulletin-add-btn").click(function() {
        if (canCreateBulletin == 0) {
            ...
            $('.bulletin-items-continer').prepend(`<div class="bulletin-item" data-id="${canCreateBulletin}">
                ...
                <div class="bulletin-id">Processo: ${BulletinId}</div>
    ...
    $(".manage-profile-save").click(function() {
        if (canSaveProfile == true) {
            ...
            setTimeout(() => {
                $(".manage-profile-save").empty();
                $(".manage-profile-save").html("Guardar");
                canSaveProfile = true
            }, 750);
            setTimeout(() => {
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function() {
        let tempalte = "[Insira o resumo do relatório aqui]\n\nArmas/itens confiscados:\n\n"
        $("#manage-incidents-title-input").val("TITULO - " + $(".date").html());
        ...
        $(".manage-incidents-title-holder").prepend(
            `
            <div class="manage-incidents-title">Gerir Ocorrências</div>
        ...
        $(".manage-incidents-editing-title").html("Você está criando uma nova Ocorrência");
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-new", function() {
        ...
        $(".manage-bolos-editing-title").html("Estás a criar nova procura")
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-save", function() {
        let existing = !($(".manage-bolos-editing-title").html() == 'Estás a criar nova procura')
    ...
    $(".icidents-person-search-holder").on("click", ".icidents-person-search-item", function() {
        ...
        $(".incidents-ghost-holder").prepend(
            ...
            <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}">
                ...
                <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">Julgado</div>
                ...
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Sentença Meses:</div>
    ...
    $(".contextmenu").on("click", ".view-AV", function() {
		
		$.post(`https://${GetParentResourceName()}/escape`, JSON.stringify({}));
		$.post(`https://${GetParentResourceName()}/verAV`, JSON.stringify({
			id: $(this).data("info")			
		}));
    ...
    $(".contextmenu").on("click", ".toggle-duty", function() {
        ...
        if (currentStatus == "Disponível") {
            $(`[data-id="${info}"]`).find(".unit-status").html("Ocupado")
            ...
            $(`[data-id="${info}"]`).find(".unit-status").html("Disponível")
    ...
    function JobColors(sentJob, seclevel) {
        if (sentJob) {
            //if (sentJob == 'police') {
                ...
                $(".header-title").html("Sistema Integrado de Informação Portucalense");
                $(".bolo-nav-item").html("Procurados");
                $(".bolos-search-title").html("Procurados");
                $("#bolos-search-input").attr('placeholder', 'Busca nos Procurados...');
                $(".manage-bolos-title").html("Gerir Procurados");
                $(".manage-bolos-editing-title").html("Estás a criar um novo procurado");
                $(".boloplate-title").html("Matrícula");
                $(".boloowner-title").html("Proprietário");
                $(".boloindividual-title").html("Indivíduo");
                $("#boloplate").attr('placeholder', 'Matrícua da viatura...');
                $("#bolodetail").attr('placeholder', 'Detalhes...');
                $("#boloowner").attr('placeholder', 'Proprietário da viatura...');
                $("#boloindividual").attr('placeholder', 'Indivíduo a ser procurado...');
                $("#home-warrants-container").fadeIn(0)
                $("#home-reports-container").fadeOut(0)           
                $(".incidents-nav-item").show()
                $(".bolo-nav-item").show()
                $(".dmv-nav-item").show()
                $(".dispatch-title-ofsomesort").html("Anúncios")
                ...
                $("#reports-officers-involved-tag-title").html("Agentes Envolvidos")
                $(".roster-iframe").attr("src", "https://docs.google.com/spreadsheets/d/e/2PACX-1vR_6NmLFm1vXvqf7dUOWBEHWPne4qhHumSL1PbrsGluTWVs2kqXpndgJhUeyFNKuFL7ouZ9CPAEyWxF/pubhtml?widget=true&amp;headers=false")
			
			if (sentJob == 'police') {
				
			$(".quote-span").html("Pela Ordem e Pela Pátria");
			document.getElementById("lpc-container").style.display = 'flex'
			
			} else if (sentJob == 'sheriff') {
			
			$(".quote-span").html("Pela Lei e pela Grei");
			document.getElementById("lpc-container").style.display = 'flex'			
			
			} else if (sentJob == 'municipal') {
			
			$(".quote-span").html("Pela Justiça e por Portugal");
			document.getElementById("lpc-container").style.display = 'none'

			} else if (sentJob == 'ranger') {
			
			$(".quote-span").html("No mar, mais além, por bem!");
			document.getElementById("lpc-container").style.display = 'none'
			...
			if (seclevel == 1) {
				$(".header-sssstitle").html("SIIP v3.7.2 | Nível de Segurança Charlie [NS-C]");	
			} else if (seclevel == 2) {
				$(".header-sssstitle").html("SIIP v3.7.2 | Nível de Segurança Bravo [NS-B]");	
			} else if (seclevel == 3) {
				$(".header-sssstitle").html("SIIP v3.7.2 | Nível de Segurança Alpha [NS-A]");
			} else if (seclevel == 4) {
				$(".header-sssstitle").html("SIIP v3.7.2 | Nível de Segurança Ómega [NS-O]");
			} else {
				$(".header-sssstitle").html("SIIP v3.7.2");
			}		
    ...
    window.addEventListener('message', function(event) {
        ...
        if (e.type == "show") {
            ...
            $(".manage-profile-editing-title").html("Você está a editar o perfil de : " + table["firstname"] + " " + table["lastname"])
            $(".manage-profile-citizenid-input").val(table["id"]);
            $(".manage-profile-name-input-1").val(table["firstname"] + " " + table["lastname"])
            $(".manage-profile-name-input-2").val(table["sex"])
            ...
            $('.licenses-holder').prepend(
                ...
                <div class="license-tag ${driver} driver">Carta de Condução A1</div>       
                <div class="license-tag ${hunting} hunting">Carta de Condução B1</div>
                <div class="license-tag ${fishing} fishing">Carta de Condução C/D</div>
                <div class="license-tag ${weapon} weapon">Porte de Arma</div>
                <div class="license-tag ${pilot} pilot">PPL(A)</div>
                <div class="license-tag ${bar} bar">Licença Advocacia</div>
                <div class="license-tag ${business} business">Carta de Navegador</div>
            `);
            $(".tags-holder").empty();
            ...
            if (table["vehicles"]) {
                $.each(table["vehicles"], function(index, value) {
                    $('.vehs-holder').prepend(`<div class="veh-tag" data-plate="${value.plate}">${value.plate}</div>`);  //- ${value.model}
                })
            }
            ...
            $.each(table, function(index, value) {
                ...
                <div class="profile-id">NIF: ${value.id}</div>
                </div>
                </div>
                </div>
                `);
            ...
            $.each(e.data, function(index, value) {
                ...
                <div class="bulletin-bottom-info">
                    <div class="bulletin-id">Processo: ${value.id}</div>
            ...
            $('.warrants-items').prepend(`<div class="warrants-item" data-id=${value.linkedincident} data-cid=${value.cid} data-name=${value.name}><div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 0.75vh; width: 100%;">
                ...
                <div class="warrant-bottom-info">
                    <div class="warrant-id">Processo: ${value.linkedincident}</div>
            ...
            $.each(table, function(index, value) {
                $('.incidents-items').prepend(
                    ...
                    <div class="incidents-top-holder">
                        ...
                        <div class="incedent-report-name">Ocorrência</div>
                    </div>
                    <div class="incidents-bottom-holder">
                        <div class="incedent-report-id">Processo: ${value.id}</div>
                        <div class="incedent-report-time-ago">Autoridade - ${value.author} - ${timeAgo(Number(value.time))}</div>
                    </div>
                </div>`
                )
            })
            ...
            for (let i = 0; i < Object.entries(penalcode).length; i++) {
                ...
                $(`#penal-${i}`).append(`
                    ...
                    <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-bottom: 0vh; padding-top: 0.75vh;">
                        ...
                        <div class="offfense-item-months">${v.months} Meses - €${v.fine} - 0 Points(s)</div>
                    </div>
                    `)
            }
            ...
            $(".incidents-ghost-holder").html("")
            $(".associated-incidents-tags-holder").html("")
            $(".manage-incidents-editing-title").html("Você está a editar a Ocorrência : Processo " + table["id"]);
                ...
                if (value.associated == true) {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            <div class="associated-incidents-user-title">NOME: ${value.name} / NIF: ${value.cid}</div>
                            <div class="associated-incidents-user-tags-holder">
            ...
            $.each(table, function(index, value) {
                ...
                $(".icidents-person-search-holder").prepend(
                    `
                    <div class="icidents-person-search-item" data-info="${name} (NIF: ${value.id})" data-cid="${value.id}" data-name="${name}">
                        <img src="${value.profilepic}" class="icidents-person-search-item-pfp">
                        <div class="icidents-person-search-item-right">
                            <div class="icidents-person-search-item-right-cid-title">NIF:</div>
            ...
            let table = e.data
            $(".manage-bolos-editing-title").html("Você está a editar Procurados , processo : " + table["dbid"]);
            ...
            $.each(table, function(index, value) {
                $('.bolos-items').prepend(
                    ...
                    <div class="bolo-bottom-holder">
                        <div class="bolo-report-id">Processo: ${value.dbid}</div>
            ...
            $(".manage-reports-editing-title").html("Você está a editar o Relatório , processo : " + id);
            ...
            $.each(table, function(index, value) {
                $('.reports-items').prepend(
                    ...
                    <div class="reports-bottom-holder">
                        <div class="reports-report-id">Processo: ${value.dbid}</div>
                    ...
                    <div style="display: flex; flex-direction: column;">
                        <div class="dmv-item-title">${value.model} | ${value.plate}</div>
                            <div class="dmv-tags">
                                ...
                                <div class="dmv-tag ${bolo}">Procurado</div>
                        ...
                        <div class="dmv-id">NIF: ${value.id} </div>
                        </div>
                    </div>
                </div>
                `) 
            ...
            $(".vehicle-info-class-input").val(table["id"]);
            ...
            $.each(lspd, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    ...
                    <div class="unit-job active-info-job-lspd">PSP</div>
            ...
            $.each(bcso, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-bcso">PM</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
            ...
            $.each(sahp, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-sahp">PMA</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
            ...
            $.each(doc, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-doc">MP</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
            ...
            $.each(ems, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-bcso">GNR</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
...
function timeShit() {
    ...
    date = localDate.toLocaleDateString('en-US', { timeZone: 'Africa/Algiers' });
    time = localDate.toLocaleTimeString('en-US', { timeZone: 'Africa/Algiers' });
    $(".date").html(date)
    $(".time").html(time)
}
...
function alterarvisao() {
	...
	document.getElementById("police-count").style.color = "grey"
	document.getElementById("ems-count").style.color = "grey"
	document.getElementById("doc-count").style.color = "grey"
	document.getElementById("pa-count").style.color = "grey"
	document.getElementById("pma-count").style.color = "grey"

	if (showSAHPPM == true) {
		document.getElementById("pma-count").style.color = "white"
		$.each(backupSAHPPM, function(index, value) {
			var status = "Disponível"
			var statuscolor = "green-status"
			var radio = "0"
			var callsign = "000"
			...
			<div class="active-unit-item" data-id="${value.cid}">
				<div class="unit-status ${statuscolor}">${status}</div>
				<div class="unit-job active-info-job-sahpPM">PMA</div>
				<div class="unit-name">(${callsign}) ${value.name}</div>
				<div class="unit-radio">${radio}</div>
			</div>
			`)
	...
	if (showSAHP == true) {
		document.getElementById("pa-count").style.color = "white"
		$.each(backupSAHP, function(index, value) {
			var status = "Disponível"
			var statuscolor = "green-status"
			var radio = "0"
			var callsign = "000"
			...
			<div class="active-unit-item" data-id="${value.cid}">
				<div class="unit-status ${statuscolor}">${status}</div>
				<div class="unit-job active-info-job-sasp">PM</div>
				<div class="unit-name">(${callsign}) ${value.name}</div>
				<div class="unit-radio">${radio}</div>
			</div>
			`)
	...
	if (showSASP == true) {
	document.getElementById("doc-count").style.color = "white"	
    $.each(backupSASP, function(index, value) {
        var status = "Disponível"
        var statuscolor = "green-status"
        var radio = "0"
        var callsign = "000"
        ...
        <div class="active-unit-item" data-id="${value.cid}">
            <div class="unit-status ${statuscolor}">${status}</div>
            <div class="unit-job active-info-job-sahp">${value.job}</div>
            <div class="unit-name">(${callsign}) ${value.name}</div>
            <div class="unit-radio">${radio}</div>
        </div>
        `)
	...
	if (showBCSO == true) {
	document.getElementById("ems-count").style.color = "white"
    $.each(backupBSCO, function(index, value) {
        var status = "Disponível"
        var statuscolor = "green-status"
        var radio = "0"
        var callsign = "000"
	...
	if (showLSPD == true) {
	document.getElementById("police-count").style.color = "white"
    $.each(backupLSPD, function(index, value) {
        var status = "Disponível"
        var statuscolor = "green-status"
        var radio = "0"
        var callsign = "000"

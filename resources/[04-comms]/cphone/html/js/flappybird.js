...
************************/
var cvs         //  canvas
var flappy_ctx         //  context'2d'
var theme1      //  original theme
var theme2      //  original them v2
var bg          //  background
var bird        //  bird: yellow
var bird1       //  bird: red
var bird2       //  bird: blue
var pipes       //  top and bottom pipes
var ground      //  ground floor
var getReady    //  get ready screen
var gameOver    //  game over screen
var map         //  map of number images
var score       //  score counter
var gameState   //  state of game
var frame       //  ms/frame = 17; dx/frame = 2; fps = 59;
var degree      //  bird rotation degree
var inFlappyGame = false;

cvs = document.getElementById('flappybird-game')
flappy_ctx = cvs.getContext('2d')
flappy_ctx.imageSmoothingEnabled = false;
theme1 = new Image()
...
bg = {
    ...
    render: function() {
        flappy_ctx.drawImage(theme1, this.imgX,this.imgY,this.width,this.height, this.x,this.y,this.w,this.h)

        //image repeat and tile to fit canvas
        flappy_ctx.drawImage(theme1, this.imgX,this.imgY,this.width,this.height, this.x + this.width,this.y,this.w,this.h)

        //image repeat again for continuous animation
        flappy_ctx.drawImage(theme1, this.imgX,this.imgY,this.width,this.height, this.x + this.width*2,this.y,this.w,this.h)
    },
...
pipes = {
    ...
    render: function() {
        ...
        for (let i = 0; i < this.pipeGenerator.length; i++) {
            ...
            flappy_ctx.drawImage(theme2, this.top.imgX,this.top.imgY,this.width,this.height, pipe.x,topPipe,this.w,this.h)
            flappy_ctx.drawImage(theme2, this.bot.imgX,this.bot.imgY,this.width,this.height, pipe.x,bottomPipe,this.w,this.h)
        }
    },
    position: function() {
    ...
    render: function() {
        if (gameState.current == gameState.play ||
            ...
            //if current score has ones, tens, and hundreds place value only
                ...
                flappy_ctx.drawImage(theme2, map[ones].imgX,map[ones].imgY,map[ones].width,map[ones].height, ( (this.x-this.w/2) + (this.w) + 3 ),this.y,this.w,this.h)

                flappy_ctx.drawImage(theme2, map[tens].imgX,map[tens].imgY,map[tens].width,map[tens].height, ( (this.x-this.w/2) ),this.y,this.w,this.h)

                flappy_ctx.drawImage(theme2, map[hundreds].imgX,map[hundreds].imgY,map[hundreds].width,map[hundreds].height, (   (this.x-this.w/2) - (this.w) - 3 ),this.y,this.w,this.h)

            //if current score has ones and tens place value only
            } else if (this.current >= 10) {
                flappy_ctx.drawImage(theme2, map[ones].imgX,map[ones].imgY,map[ones].width,map[ones].height, ( (this.x-this.w/2) + (this.w/2) + 3 ),this.y,this.w,this.h)

                flappy_ctx.drawImage(theme2, map[tens].imgX,map[tens].imgY,map[tens].width,map[tens].height, ( (this.x-this.w/2) - (this.w/2) - 3 ),this.y,this.w,this.h)
            
            //if current score has ones place value only
            } else {
                flappy_ctx.drawImage(theme2, map[ones].imgX,map[ones].imgY,map[ones].width,map[ones].height, ( this.x-this.w/2 ),this.y,this.w,this.h)
            }
        }
    }
}    
...
bird = {
    ...
    render: function() {
        ...
        flappy_ctx.translate(this.x, this.y)
        //rotate bird by degree
        flappy_ctx.rotate(this.rotation)
        flappy_ctx.imageSmoothingEnabled = true;
        flappy_ctx.drawImage(theme1, bird.imgX, bird.imgY, this.width, this.height, -this.w/2, -this.h/2, this.w, this.h)
        flappy_ctx.imageSmoothingEnabled = false;
        flappy_ctx.restore()
        //bird is centered on x,y position
        // flappy_ctx.drawImage(theme1, bird.imgX,bird.imgY,this.width,this.height, this.x-this.w/2,this.y-this.h/2,this.w,this.h)
    },
    //bird flies
    flap: function() {
...
bird1 = {
    ...
    render: function() {
        let bird = this.animation[this.fr]
        //bird is centered on x,y position
        flappy_ctx.drawImage(theme2, bird.imgX,bird.imgY,this.width,this.height, this.x-this.w/2,this.y-this.h/2,this.w,this.h)
    },
    //bird flies
    flap: function() {
        this.velocity = - this.fly
...
let draw = () => {
    // limpa o canvas
    flappy_ctx.fillStyle = '#00bbc4';
    flappy_ctx.fillRect(0, 0, cvs.width, cvs.height);
    // desenha os elementos
    bg.render();
    pipes.render();
    ground.render();
    
    // desenha o passarinho e telas de Ready/Game Over
    bird.render();
    getReady.render();
    gameOver.render();
    
    // renderiza o score por último para ficar sobre os demais elementos
    score.render();
}
...
function OpenFlappyGame() {
    ...
    loop();
    intervalId = setInterval(loop, 17);
}

function ExitFlappyGame() {
    inFlappyGame = false;
...
score = {
    ...
    w: 15,
    h: 25,
    scoreTextOffset: { x: 70, y: -125 }, // o x é positivo para a direita e o y é positivo para baixo se for negativo é para cima
    bestTextOffset: { x: 70, y: -110 },
    reset: function() {
        this.current = 0;
    },
    render: function() {
        if (gameState.current == gameState.play ||
            ...
            if (this.current >= 1000) {
                ...
                flappy_ctx.drawImage(
                    theme2,
                    map[ones].imgX, map[ones].imgY,
                    map[ones].width, map[ones].height,
                ...
                flappy_ctx.drawImage(
                    theme2,
                    map[tens].imgX, map[tens].imgY,
                    map[tens].width, map[tens].height,
                    ((this.x - this.w / 2)),
                    this.y,
                    this.w, this.h
                );
                flappy_ctx.drawImage(
                    theme2,
                    map[hundreds].imgX, map[hundreds].imgY,
                    map[hundreds].width, map[hundreds].height,
                    ((this.x - this.w / 2) - (this.w) - 3),
                    this.y,
                    this.w, this.h
                );
        ...
        if (gameState.current == gameState.gameOver) {
            ...
            flappy_ctx.font = "20px Arial";
            flappy_ctx.fillStyle = "#000";
            flappy_ctx.textAlign = "center";
            flappy_ctx.strokeStyle = "#000";
            flappy_ctx.fillText(
                "" + this.current, // Score atual
                cvs.width / 2 + this.scoreTextOffset.x,
                cvs.height / 2 + 60 + this.scoreTextOffset.y
            );
            flappy_ctx.fillText(
                "" + this.best, // Melhor Score
                cvs.width / 2 + this.bestTextOffset.x,
                cvs.height / 2 + 90 + this.bestTextOffset.y
            );
        }
        }
    }

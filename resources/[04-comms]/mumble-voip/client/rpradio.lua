local radioConfig = {
	...
	Frequency = {
		...
		Max = 8000, -- Max number of frequencies
...
Radio.Labels = {        
	...
	{ "FRZL_RADIO_INPUT", "Insere uma frequência" },
}
Radio.Commands = {
	{
		...
		Handler = function(src, args, raw)
			...
			elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
				...
				Radio.On = false
				Radio:Remove()
				setVoiceProperty("radioEnabled", false)
			elseif Radio.Open and isFalling then
				Radio:Toggle(false)
			end   
			
			if not Radio.Has then
				exports["cframework"]:SendAlert("error", 'Não tens um radio')
			end
		end,
	},
	{
...
function Radio:Toggle(toggle)
	...
	if self.On and not radioConfig.AllowRadioWhenClosed then
		setVoiceProperty("radioEnabled", toggle)
	end
	...
	if self.Open then
		TriggerEvent("attachItem", "radio01")
...
function SetAllowRadioWhenClosed(value)
	...
	if Radio.On and not Radio.Open and radioConfig.AllowRadioWhenClosed then
		setVoiceProperty("radioEnabled", true)
	end
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local isBroadcasting = radioPressed
		...
		if isActivatorPressed and isSecondaryPressed and not Radio.Has then
			exports["cframework"]:SendAlert("error", 'Não tens um radio')
		end
		...
		if Radio.Open then
			...
			if not Radio.On then
				AddTextComponentSubstringPlayerName(Radio.Clicks and "~r~desligar~w~" or "~g~ligar~w~")
			end
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				...
				setVoiceProperty("radioEnabled", Radio.On)

				if Radio.On then
					--PlaySoundSingleJS('on.ogg', 0.3 )
					--SendNUIMessage({ sound = "audio_on", volume = 0.3})
					Radio:Add(radioConfig.Frequency.Current)
				else
					--PlaySoundSingleJS('off.ogg', 0.5 )
			...
			if not Radio.On then
				...
				if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
					...
					setVoiceProperty("micClicks", Radio.Clicks)
				end
			end
		else
...
AddEventHandler("onClientResourceStart", function(resName)
	...
	setVoiceProperty("radioClickMaxChannel", radioConfig.Frequency.Max) -- Set radio clicks enabled for all radio frequencies
	setVoiceProperty("radioEnabled", false) -- Disable radio control

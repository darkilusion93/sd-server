ESX = nil
TriggerEvent('esx:getShbuedaloucoaredObjbuedaloucoect', function(obj) ESX = obj end)
...
local function radioNameGetter_orig(source)
	return ESX.getFirstname(source).. " " ..ESX.getLastname(source).. " | "..source
end
...
function addPlayerToRadio(source, radioChannel)
	...
	for player, _ in pairs(radioData[radioChannel]) do
		...
		TriggerClientEvent("esx:showNotification", player, radioNameGetter(source).. " Entrou Na Radio", "success")
	end
end
...
function removePlayerFromRadio(source, radioChannel)
	...
	radioData[radioChannel] = radioData[radioChannel] or {}
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('pma-voice:removePlayerFromRadio', player, source, radioNameGetter(source), 0)
		if player ~= source then
			TriggerClientEvent("esx:showNotification", player, radioNameGetter(source).. " Saiu da Radio", "error")
		end
	end
...
function setTalkingOnRadio(talking)
	...
	if radioTbl then
		...
		for player, _ in pairs(radioTbl) do
			if talking then
				table.insert(talkinglist, radioNameGetter(source))
				data = {
					action = "START",
					id = source,
					type = "inform",
					text = "A transmitir: " ..radioNameGetter(source),
					style = "inform",
				}
				TriggerClientEvent("mythic_notify:client:PersistentAlert", player, data)
				TriggerClientEvent("UpdateSpeakers", player, talkinglist, plyVoice.radio)
			else
				talkinglist = removeByValue(talkinglist, radioNameGetter(source))
				data = {
					action = "END",
					id = source,
					type = "inform",
...
function GetPlayersIdInRadioChannel(source)
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	voiceData[source] = voiceData[source] or defaultTable(source)
	local plyVoice = voiceData[source]
	local radioTbl = radioData[plyVoice.radio]
...
exports('GetPlayersIdInRadioChannel', GetPlayersIdInRadioChannel)

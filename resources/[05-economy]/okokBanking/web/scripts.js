			...
			if (selectedWindow == "none") {
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div class="ldata-txt">Conectando...</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
					...
					$('#menu').html(`
						...
						<div class="col-md-10 tab-s">
							<div class="d-flex justify-content-between align-items-center">
								<span class="selected-page"><span id="page-title">Vista Geral</span></span>
								<div>
			...
			for(var i = 0; i < num; i++) {
				...
				if (db.type == 'transfer' && db.receiver_identifier == event.data.identifier) {
					...
					data = `<td class="align-middle fw500">
								...
								<div class="mtm3125">Recebido</div>
							</td>`;
				...
				// Deposited
					...
					data = `<td class="align-middle fw500">
								Para <span class="transactions-name">${name}</span>
								<div class="mtm3125">Depositado</div>
							</td>`;
					amount = `<td class="align-middle fw500 transactions-received text-center">+ ${db.value.toLocaleString()} EUR</td>`;
				// Withdrawn
					...
					data = `<td class="align-middle fw500">
								De <span class="transactions-name">${name}</span>
								<div class="mtm3125">Levantado</div>
							</td>`;
					amount = `<td class="align-middle fw500 text-center">- ${db.value.toLocaleString()} EUR</td>`;
				}
					...
					<div class="col col-md-3 pr05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">MOVIMENTOS</h6>
...
function overview_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "bankmenu" || !event.data.isUpdate){
		...
		if(event.data.RequireCC) {
			informations = `<span class="card-o-title">Informações<span class="badge bg-primary viewall-badge buy_new_card" data-bs-toggle="modal" data-bs-target="#buycc_modal"><i class="fa-solid fa-plus"></i> Comprar</span></span>`;
		} else {
			informations = `<span class="card-o-title">Informações</span>`;
		}
			...
			<div class="col-md-7 chart-main">
				<div class="card chart-card text-center">
					<div class="card-header card-o-header">
						<span class="card-o-title">Estatisticas</span>
					</div>
					...
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Movimentos Recentes<span class="badge bg-primary viewall-badge" id="view_all_transactions"><i class="fas fa-eye"></i> Ver Tudo</span></span>
							</div>
						...
						<div class="card-body ccard-body">
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
									<span class="d-flex justify-content-between"><span><img src="img/visa_white.svg" class="w20p"><span class="ccard-name">Portugal Classic</span></span><span><i class="fas fa-wifi"></i></span></span>
									<div class="mt38p">
										<div class="d-flex align-items-center">
											<span class="ccard-status">Portugal</span>
										</div>
										<div class="d-flex justify-content-between align-items-center">
											<span class="ccard-active">BANK</span>
											<div class="d-flex align-items-center ccard-valid">
												<span class="ccard-thru">VALID</span>
							...
							<hr>
							<p class="card-text text-center ccard-fs" style="margin-bottom: 1.1rem;"><span class="fff">Montante:</span> <span id="playerBankMoney"></span> EUR</p>
		...
		const months = ["Jan", "Feb", "Mar", "Abr", "Mai", "Jun", "Jul", "Ag", "Set", "Out", "Nov", "Dez"];
	...
	if(event.data.isUpdate && selectedWindow == "societies" || !event.data.isUpdate){
		...
		if (event.data.isInSociety){
			society = `<span class="sidebar-title mt-5">Empresa</span>
					   <p class="sidebar-item mt-2 selected" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Vista Geral</span></p>
					   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Movimentos</span></p>`;
		} else {
			society = '';
		}

		$('#sidebar').html(`
			<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Vista Geral</span></p>
			<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Movimentos</span></p>
			...
			<div class="col-md-7 chart-main">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Last Transactions<span class="badge bg-primary viewall-badge" id="view_all_transactions_society"><i class="fas fa-eye"></i> Ver Tudo</span></span>
							</div>
							<div class="card-body" id="last-t-body">
								<table id="lastTransactionsTable">
									<tbody id="lastTransactionsData">
...
function settings_page_function(event) {
	...
	if (event.data.isInSociety){
		society = `<span class="sidebar-title mt-5">Empresa</span>
				   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Vista Geral</span></p>
				   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Movimentos</span></p>`;
	} else {
		society = '';
	}
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Vista Geral</span></p>
		<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Movimentos</span></p>
		<p class="sidebar-item mt-2 selected" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Configurações</span></p>
		${society}
	`);
							...
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changepin-card">
									<div class="card-body text-center">
										...
										<button type="button" id="change_pin" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Confirmar</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Aletração no avlor de: ${event.data.pinCost}€</span>
										<hr class="fff">
										<span>Maximo de Numeros é: ${event.data.pinCharNum}</span>
										<hr class="fff">
										<span>Só Pode ser usado Numeros</span>
									</div>
								</div>
							</div>
						</div>

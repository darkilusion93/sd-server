local afkTimeout = 3600*2 -- AFK kick time limit in seconds (2 hours)
local timer = 0
...
Citizen.CreateThread(function()
	while true do
		...
		if playerPed then
			...
			if not hurt and health <= 160 then
				setHurt()
			...
			if currentPosition == previousPosition and currentHeading == previousHeading then
				if timer > 0 then
					if timer == math.ceil(afkTimeout / 4) then
						TriggerEvent('chat:addMessage', { args = { 'AFK: Vais ser kickado por inatividade em breve' } })
					end

...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, DecorGetFloat(vehicle, Config.FuelDecor) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10)
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if pumpDistance < 2.5 then
			if Config.UseESX and not isNearPump then
				ESX.TriggerServerCallback('getFuelMoney', function(money)
					currentCash = money
				end)
			end
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100 >= 0 then
                ...
                TriggerEvent("cframework:forceUpdateWeaponAmmo", true)
			else
				isFueling = false
			end
		else
	...
	if pumpObject then
        ...
        TriggerEvent("cframework:forceUpdateWeaponAmmo", false)
	end
				...
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								TriggerServerEvent('cframework:buyJerrycan')
...
function CreateBlipFuel(coords)
	...
	BeginTextCommandSetBlipName("FUELBLIP")
...
if Config.ShowNearestGasStationOnly then
	Citizen.CreateThread(function()
		...
		while true do
			...
			currentGasBlip = CreateBlipFuel(closestCoords)
		end
	end)
elseif Config.ShowAllGasStations then
	Citizen.CreateThread(function()
		AddTextEntry('FUELBLIP', 'Bomba de Gasolina')
...
function SetFuel(vehicle, fuel)
	if type(fuel) == 'number' and fuel >= 0 and fuel <= 100 then
        ...
        if NetworkDoesNetworkIdExist(netId) then
            TriggerServerEvent('cframework:syncFuel', netId, fuel)
        end
	end
end

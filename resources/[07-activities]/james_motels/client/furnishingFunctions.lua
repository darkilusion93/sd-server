...
OpenControlMenu = function()
    local menuElements = {
        {
            ["label"] = "Coloque um novo móvel.",
            ["action"] = "furnish"
        },
        {
            ["label"] = "Gerencie móveis já existentes.",
            ["action"] = "refurnish"
        }
    }
...
OpenFurnishingMenu = function()
    ...
    if not cachedData["furnishings"][currentMotel] then return ESX.ShowNotification("Você não tem nenhum móvel comprado.") end
    if not cachedData["furnishings"][currentMotel]["ownedFurnishing"] or #cachedData["furnishings"][currentMotel]["ownedFurnishing"] == 0 then return ESX.ShowNotification("Você não tem nenhum móvel comprado.") end
...
OpenRefurnishMenu = function()
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "refurnishing_motel_room", {
        ["title"] = "Escolha móveis para gerenciar.",
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
        local action = menuData["current"]["action"]
        ...
        elseif keyLabel == "ENTER" then
            ...
            ESX.TriggerServerCallback("james_motels:saveFurnishing", function(savedFurnishing)
                if savedFurnishing then
                    ...
                    ESX.ShowNotification("Você colocou os móveis.")
                else
                    ESX.ShowNotification("Por favor, tente colocá-lo novamente.")
                end
...
OpenFurnishingComputer = function()
    ...
    if not ownedMotel then return ESX.ShowNotification("Você precisa ter um quarto de motel para gerenciar o computador.") end
...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("james_motels:purchaseFurnishing", function(purchased)
            DrawBusySpinner("Verificando a compra...")
            ...
            if purchased then
                ESX.ShowNotification("Compra bem sucedida, entregue no seu quarto.")
            else
                ESX.ShowNotification("A autenticação falhou, verifique o seu saldo.")
            end
    ...
    Citizen.CreateThread(function()
        while DoesEntityExist(currentProp["handle"]) do
            ...
            DrawScriptText((cachedData["furnishingProp"]["coords"] or Config.MegaMall["object"]["pos"]) - vector3(0.0, 0.0, 0.25), "[~g~G~s~] para confirmar a compra.")

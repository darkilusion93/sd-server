...
local function canVehicleBeTuned(v, veh)
    if v.isNautica then
        if GetVehicleTypeRaw(veh) ~= 1 and GetVehicleTypeRaw(veh) ~= 8 and GetVehicleTypeRaw(veh) ~= 13 and GetVehicleTypeRaw(veh) ~= 15 then
            ESX.ShowNotification('Só podes tunar barcos, helicopteros e aviões.', "error")
            return false
        end
    ...
    if v.isMotoclub then
        if GetVehicleTypeRaw(veh) ~= 3 and GetVehicleTypeRaw(veh) ~= 11 then
            ESX.ShowNotification('Só podes tunar motas.', "error")
            return false
        end
    ...
    if GetVehicleTypeRaw(veh) == 11 or GetVehicleTypeRaw(veh) == 3 or GetVehicleTypeRaw(veh) == 1 or GetVehicleTypeRaw(veh) == 8 or GetVehicleTypeRaw(veh) == 13 or GetVehicleTypeRaw(veh) == 15 then
        ESX.ShowNotification('Só podes tunar carros.', "error")
        return false
    end
...
Citizen.CreateThread(function()
	...
	while true do 
		...
		if nomidaberto == nil then
			for i = 1, #ESX.TunningLocations do
				local v = ESX.TunningLocations[i]
				if (myjob.name == v.job or v.job == nil) and not v.used then
					...
					if distance <= Config.Range+2 then
						...
						if distance <= Config.Range then
							dormir = 5
							if not nomidaberto and IsPedInAnyVehicle(PlayerPedId(), false) then
								ESX.ShowHelpNotification('Pressiona ~INPUT_PICKUP~ para personalizar o veiculo.')
							end
							
							if IsControlJustReleased(0, 38) and IsPedInAnyVehicle(PlayerPedId(), false) then
								local veh, distance = ESX.Game.GetClosestVehicle(v.coords)
								if DoesEntityExist(veh) and distance <= Config.Range then
                                    ...
                                    --if canVehicleBeTuned(v, carroselected) then
                                        ...
                                        local tabelainfo = ModsAvailable(veh, v.illegal)
...
function ModsAvailable(carro, illegal)
	local mods = {}
	TunagensDefault = {}
	preco = 0
    ...
    mods[#mods+1],stancetable = Stance()
    TunagensDefault["stance"] = stancetable
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "subMenuAction" then  -- aqui é quando o gajo clica para que nivel quer tunar
		...
		elseif menuatual == "stance" then
            if not stance then
                AddMoneyNotDefault(3,Config.TunningPrices["Stance"],2,3)
                stance = true
            end

            if tab.name == "frontTrackWidth" then
                ESX.Game.SetVehicleProperties(carroselected, {frontTrackWidth = tonumber(string.format("%.4f", tab.value))})
            elseif tab.name == "backTrackWidth" then
                ESX.Game.SetVehicleProperties(carroselected, {backTrackWidth = tonumber(string.format("%.4f", tab.value))})
            elseif tab.name == "frontTrackRotation" then
                ESX.Game.SetVehicleProperties(carroselected, {frontTrackRotation = tonumber(string.format("%.4f", tab.value))})
            elseif tab.name == "backTrackRotation" then
                ESX.Game.SetVehicleProperties(carroselected, {backTrackRotation = tonumber(string.format("%.4f", tab.value))})
            elseif tab.name == "wheelWidth" then
                ESX.Game.SetVehicleProperties(carroselected, {wheelWidth = tonumber(string.format("%.4f", tab.value))})
            end
    ...
    elseif data.action == "subSubMenuAction" then  -- aqui é quando o gajo clica para que nivel quer tunar
        ...
        elseif tab.tipo == "corspecial" then
            local price = Config.TunningPrices["PrimaryColorType"]
            local def = TunagensDefault["corprinc"].tipao
            local ptp, colorp = GetVehicleColours(carroselected)
            AddMoneyNotDefault(def,price,tab.index,ptp)
		...
		elseif tab.tipo == "drift" then
            ...
            ESX.Game.SetVehicleProperties(carroselected, {drift = not costum})
    ...
    elseif data.action == "finish" then-- quando acabar
		if preco > 0 then
			TriggerServerEvent("ctunning:applyMods",preco,nomidaberto,ESX.Game.GetVehicleProperties(carroselected))
		else
		end
...
function CancelEverything(data)
	for k,v in pairs(TunagensDefault) do
        ...
        elseif k=="stance" then
            SetVehicleWheelWidth(carroselected, cenas.wheelWidth)
            TriggerServerEvent("cframework:syncStance", NetworkGetNetworkIdFromEntity(carroselected), cenas.frontTrackWidth, cenas.backTrackWidth, cenas.frontTrackRotation, cenas.backTrackRotation)
...
RegisterNetEvent('ctunning:modSuccess', function(pago)
	if not pago then
		CancelEverything()
	end
	chegoupago = true

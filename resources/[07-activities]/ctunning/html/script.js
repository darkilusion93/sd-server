...
$(document).on('click', ".menu-item", function() {
    ...
    if (type == "main-menu") {
        $("#item-wrapper").css("grid-template-columns", "repeat(4, 5.6vw)")
        currentSubMenu = menuTable[index].subMenu;
        $.post('https://ctunning/action', JSON.stringify({
            action: "openSubMenu",
		...
		$.each(subMenu, function(index, value) {
            ...
            var newItemDiv = /*html*/`
                <div class="menu-item ${sel}" data-type="${value.type}" data-subsubindex="${index2}" data-index="${index}" data-menuType = "${menuType}">
                    <div class="item-icon-wrapper">
                        <Img class="${icon}" src="${value.src}"></Img>
                    </div>
...
function openSecondPage(index) {
	...
	$.each(subMenu, function(index2, value) {
        ...
        if (value.subSubMenu) {
            ...
            if (value.slider === undefined) {
                ...
                var newItemDiv = /*html*/`
                    <div class="menu-item ${sel}" data-type="${value.type}" data-index="${index2}" data-menuType = "${menuType}"><div class="item-icon-wrapper">
                        <Img class="${icon}" src="${value.src}"></Img></div>
                        <div class="item-name">${value.title}</div>
                    </div>`;
                $("#item-wrapper").append(newItemDiv);
            } else {
                $("#item-wrapper").css("grid-template-columns", "repeat(1, 5.6vw)")
                var menuType = "second-menu"
                if (value.colorpicker == true) {menuType = "colorpicker"}
                
                var newItemDiv = /*html*/`
                    <div class="slider-item ${sel}" data-type="${value.type}" data-index="${index2}" data-menuType = "${menuType}">
                        <div class="item-icon-wrapper">
                            <input type="range" min="${parseFloat(value.min)}" max="${parseFloat(value.max)}" value="${parseFloat(value.default)}" step="0.001" class="stance-slider" id="${value.name}">
                        </div>
                        <div class="item-name">${value.title}</div>
                    </div>`;
                $("#item-wrapper").append(newItemDiv);
    ...
    $('.stance-slider').on('input', function() {
        ...
        $.post('https://ctunning/action', JSON.stringify({
            action: "subMenuAction",
            type: JSON.stringify({tipo: "stance", name: type, value: value}),
        }));
    });
}

...
function openMain() {
	...
	goodAnswer = [];
	questionUsed = [];
	nbQuestionToAnswer = 10; // don't forget to change the progress bar max value in html
	nbAnswerNeeded = 7; // out of nbQuestionToAnswer
	nbPossibleQuestions = 10; // number of questions in database questions.js
	lastClick = 0;
...
function openQuestionnaire() {
	...
	$("#questionNumero").html("Questão: " + questionNumber);
...
$(".btnQuestion").click(function () {
	$.post('http://cdrivingschool/question', JSON.stringify({}));
});

$(".btnClose").click(function () {
	$.post('http://cdrivingschool/close', JSON.stringify({}));
});

$(".btnKick").click(function () {
	$.post('http://cdrivingschool/kick', JSON.stringify({}));
});
